...
// CONSTRUCTOR
Batalla::Batalla(): alumno(nullptr), profesor(nullptr),
...
// DIBUJAR
// cada clase sabe dibujarse a si misma
...
void Batalla::dibujarPiezas() {
    ...
    //como el .dibujar() de tu personaje calcula 
    ...
    //Lo movemos a la posición de la arena
    glTranslatef(xProfesor, yProfesor, 0.1f);

    // Contrarrestamos la posición fija de su fila/columna del tablero
    glPushMatrix();
...
void Batalla::actualizar(float dt) {
    ...
    //si hay 2 jugadores
    if (esJvsJ) {
        xProfesor += vxProfesor * dt;
        yProfesor += vyProfesor * dt;

        // limites de la arena para el jugador 2
...
void Batalla::moverIA(float dt) {
    ...
    if (distancia > distMin && distancia > 0.01f) {
       ...
       // yProfesor += dy * dt * velIA;
        ...
        float nuevaX = xProfesor + dx * dt * velIA;
        float nuevaY = yProfesor + dy * dt * velIA;

        // Comprobar si la nueva posicion choca con alguna piedra
        bool bloqueado = false;
        for (int i = 0; i < NUM_PIEDRAS; i++) {
            float dpx = nuevaX - piedras[i].x;
            float dpy = nuevaY - piedras[i].y;
        ...
        // Si no esta bloqueado mover, si esta bloqueado intentar rodear
        if (!bloqueado) {
            xProfesor = nuevaX;
            yProfesor = nuevaY;
        }
        else {
            // Intentar moverse solo en X o solo en Y para rodear la piedra
            float soloX = xProfesor + dx * dt * velIA;
            float soloY = yProfesor;
            bool bloqueadoX = false;
            for (int i = 0; i < NUM_PIEDRAS; i++) {
                float dpx = soloX - piedras[i].x;
                float dpy = soloY - piedras[i].y;
                if (sqrtf(dpx * dpx + dpy * dpy) < 0.5f) {
                    bloqueadoX = true;
                    break;
                }
            }
            if (!bloqueadoX) {
                ...
                float soloXb = xProfesor;
                float soloYb = yProfesor + dy * dt * velIA;
                bool bloqueadoY = false;
                for (int i = 0; i < NUM_PIEDRAS; i++) {
                    float dpx = soloXb - piedras[i].x;
                    float dpy = soloYb - piedras[i].y;
                    if (sqrtf(dpx * dpx + dpy * dpy) < 0.5f) {
                        bloqueadoY = true;
                        break;
                    }
                ...
                // Si ambos bloqueados no se mueve - la piedra le corta el paso
            }
        }
    }
...
//si un jugador es el bando oscuro
void Batalla::comprobarAtaquesProfesor() {
    if (!proyectilProfesor.getActivo()) {
        float dx = xAlumno - xProfesor;
        float dy = yAlumno - yProfesor;
        float dist = sqrtf(dx * dx + dy * dy);
...
void Batalla::comprobarFinBatalla() {
    ...
    if (!alumno->getEstaViva() && !profesor->getEstaViva()) {
        ...
        ETSIDI::play("sonidos/victoria_batalla.wav");
    }
}
...
void Batalla::gestionTeclado(unsigned char tecla) {
    ...
    switch (tecla) {
        ...
        break;
        // Jugador 2 - OSCURIDAD - IJKL + H para atacar
        case 'i': case 'I': if (esJvsJ) vyProfesor = 1.5f; break;
        case 'k': case 'K': if (esJvsJ) vyProfesor = -1.5f; break;
        case 'j': case 'J': if (esJvsJ) vxProfesor = -1.5f; break;
        case 'l': case 'L': if (esJvsJ) vxProfesor = 1.5f; break;
        case 'h': case 'H': if (esJvsJ) comprobarAtaquesProfesor(); break;
    }
