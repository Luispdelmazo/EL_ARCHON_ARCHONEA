...
Menu::Menu() {
    ...
    modoSeleccionado       = ModoJuego::JvsIA;
...
void Menu::dibujar() {
    ...
    else if (estadoMenu == EstadoMenu::MODO) {   
...
void Menu::dibujarSeleccionModo() {
    glBegin(GL_QUADS);
    glColor3f(0.05f, 0.05f, 0.1f);
    ...
    glColor3f(1.0f, 0.85f, 0.0f);
    dibujarTexto(-2.0f, 3.0f, 0.1f, "SELECCIONA MODO DE JUEGO");

    // JvsIA
    if (modoSeleccionado == ModoJuego::JvsIA) {
        glColor3f(1.0f, 0.85f, 0.0f);
    }
    else {
        glColor3f(0.6f, 0.6f, 0.6f);
    }
    dibujarTexto(-1.5f, 1.0f, 0.1f, "1 - SOLO UN JUGADOR");
    glColor3f(0.4f, 0.4f, 0.4f);
    dibujarTexto(-2.5f, 0.4f, 0.1f, "Tu controlas LUZ, la maquina mueve OSCURIDAD");

    // JvsJ
    if (modoSeleccionado == ModoJuego::JvsJ) {
        glColor3f(1.0f, 0.85f, 0.0f);
    }
    else {
        glColor3f(0.6f, 0.6f, 0.6f);
    }
    dibujarTexto(-1.5f, -0.5f, 0.1f, "2 - JUGADOR vs JUGADOR");
    glColor3f(0.4f, 0.4f, 0.4f);
    dibujarTexto(-2.5f, -1.1f, 0.1f, "Dos jugadores en el mismo teclado");
...
void Menu::dibujarSeleccionNivel() {
    ...
    glColor3f(1.0f, 0.85f, 0.0f);
    dibujarTexto(-2.0f, 3.0f, 0.1f, "SELECCIONA DIFICULTAD");
...
void Menu::gestionRaton(int boton, int estado, float glX, float glY) {
    ...
    if (estadoMenu == EstadoMenu::PRINCIPAL) {
        ...
        else if (glY > -1.5f && glY < -0.1f) {
            modoSeleccionado = ModoJuego::JvsJ;
            haSeleccionado = true;
        }
    } else if (estadoMenu == EstadoMenu::NIVEL) {
...
void Menu::gestionTeclado(unsigned char tecla) {
    ...
    if (estadoMenu == EstadoMenu::MODO) {
        if (tecla == '1') { modoSeleccionado = ModoJuego::JvsIA; estadoMenu = EstadoMenu::NIVEL; }
        if (tecla == '2') { modoSeleccionado = ModoJuego::JvsJ;  haSeleccionado = true; } // <-- directo
        if (tecla == 13)  estadoMenu = EstadoMenu::NIVEL;
